-- Casa Clara: esquema opcional para conectar la app a Supabase.
-- Ejecutar en el SQL Editor de un proyecto nuevo.
...
create table if not exists public.accounts (
  id uuid primary key default uuid_generate_v4(), household_id uuid not null references public.households(id) on delete cascade,
  owner_id uuid references auth.users(id) on delete set null, name text not null, kind text not null default 'personal' check (kind in ('personal','shared')),
  opening_balance numeric(12,2) not null default 0, created_at timestamptz not null default now()
);
create table if not exists public.transactions (
  ...
  created_by uuid not null references auth.users(id) on delete restrict, description text not null, category text not null,
  amount numeric(12,2) not null check (amount > 0), type text not null check (type in ('income','expense')), occurred_on date not null default current_date,
  created_at timestamptz not null default now()
);
create table if not exists public.recurring_payments (
  id uuid primary key default uuid_generate_v4(), account_id uuid not null references public.accounts(id) on delete cascade,
  created_by uuid not null references auth.users(id) on delete restrict, name text not null, amount numeric(12,2) not null check (amount > 0), due_day int not null check (due_day between 1 and 31), status text not null default 'pending' check (status in ('pending','paid')), created_at timestamptz not null default now()
);
...
create policy "members can add transactions" on public.transactions for insert with check (created_by = auth.uid() and public.can_access_account(account_id));
...
create policy "members can add payments" on public.recurring_payments for insert with check (created_by = auth.uid() and public.can_access_account(account_id));
...
-- Usuarios sugeridos: crear Eli y Wil en Authentication > Users y añadirlos a la misma household.
